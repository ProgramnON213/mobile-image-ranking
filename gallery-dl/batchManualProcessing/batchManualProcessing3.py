...
class ZoomableImageViewer(Toplevel):
    """A custom pop-up window that handles viewing, zooming, and panning high-res images."""
    def __init__(self, master, image_path):
        super().__init__(master)
        self.title(os.path.basename(image_path))
        self.geometry("1200x800") # Default pop-up size
        
        self.original_img = Image.open(image_path)
        self.scale = 1.0
        
        # Calculate initial scale so the image fits the screen nicely
        ...
        # Set up a Canvas with Scrollbars for panning when zoomed in
    ...
    def zoom(self, event):
        """Adjusts the scale factor based on mouse wheel movement."""
        # Check for Linux button 4/5 or Windows/Mac delta
    ...
    def update_image(self):
        """Resizes the original image based on current scale and updates the canvas."""
        new_w = int(self.original_img.width * self.scale)
        new_h = int(self.original_img.height * self.scale)
        
        # Prevent scaling down to 0 which crashes PIL
...
class ImageBatchCleaner:
    def __init__(self, root):
        self.root = root
        self.root.title("Batch Image Cleaner - High Res Pro")
        
        # 4. Enforce base resolution and attempt to auto-maximize
        self.root.geometry("1680x1050")
        try:
            self.root.state('zoomed') # Works on Windows
        except:
            pass # Fails silently on platforms where 'zoomed' isn't supported
        ...
        # Bind mouse wheel scrolling to the main canvas
    ...
    def _bind_mouse_scroll(self, widget):
        """Helper to bind scroll wheel events to a widget."""
    ...
    def _on_main_scroll(self, event):
        """Handles vertical scrolling in the main grid view."""
        if event.num == 5 or event.delta < 0:
    ...
    def load_directory(self, folder_path=None):
        ...
        for root_dir, dirs, files in os.walk(folder_path):
            ...
            # 1. Skip batches with only 1 image
            if len(img_files) > 1:
    ...
    def on_batch_select(self, event):
        ...
        for file in img_files:
            ...
            # 3. Ensure mouse scrolling works even when hovering over cards
            ...
            try:
                ...
                # 2. Open the new Zoomable Pop-up
    ...
    def show_full_image(self, path):
        try:
            # Replaced the static Toplevel with our new custom zooming class
            ZoomableImageViewer(self.root, path)
        except Exception as e:
