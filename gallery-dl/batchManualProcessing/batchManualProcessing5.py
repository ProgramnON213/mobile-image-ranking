...
class ZoomableImageViewer(Toplevel):
    """A custom pop-up window that auto-sizes to the image and allows smartphone-like interaction."""
    def __init__(self, master, image_path):
        ...
        # Get monitor dimensions with a 100px buffer so it doesn't overlap taskbars
        screen_w = self.winfo_screenwidth() - 100
        screen_h = self.winfo_screenheight() - 100
        
        # Determine the window size: Exact image size, capped at screen size
        win_w = min(img_w, screen_w)
        win_h = min(img_h, screen_h)
        
        self.geometry(f"{win_w}x{win_h}")
        
        # Calculate initial scale so the image fits the window perfectly on launch
        if img_w > screen_w or img_h > screen_h:
            self.scale = min(screen_w / img_w, screen_h / img_h)
        else:
            self.scale = 1.0

        # Canvas without scrollbars for clean viewing
        self.canvas = tk.Canvas(self, bg="#222222", highlightthickness=0)
        self.canvas.pack(fill=tk.BOTH, expand=True)

        # Anchor the image exactly in the center of the dynamic window
...
class ImageBatchCleaner:
    def __init__(self, root):
        ...
        # Main app resolution targeting your monitor
