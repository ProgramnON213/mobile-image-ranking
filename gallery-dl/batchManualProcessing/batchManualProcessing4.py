...
class ZoomableImageViewer(Toplevel):
    """A custom pop-up window optimized for smartphone-like panning and fast zooming."""
    def __init__(self, master, image_path):
        super().__init__(master)
        self.title(os.path.basename(image_path))
        self.geometry("1200x800")
        
        # Keep window size fixed, remove standard maximize/minimize flickering if desired
        self.resizable(False, False) 
        
        self.original_img = Image.open(image_path)
        self.scale = 1.0
        
        # Calculate initial scale to fit the window perfectly
        win_w, win_h = 1200, 800
        img_w, img_h = self.original_img.size
        self.scale = min(win_w / img_w, win_h / img_h)

        # Canvas without scrollbars for clean, phone-like viewing
        self.canvas = tk.Canvas(self, bg="#222222", highlightthickness=0)
        self.canvas.pack(fill=tk.BOTH, expand=True)

        self.image_id = self.canvas.create_image(win_w//2, win_h//2, anchor="center")
        
        # --- Smartphone-like Interaction Bindings ---
        # 1. Panning (Click and Drag)
    ...
    def start_pan(self, event):
        """Marks the starting point of a drag operation."""
        self.canvas.config(cursor="fleur")
        self.canvas.scan_mark(event.x, event.y)

    def do_pan(self, event):
        """Translates the drag into canvas movement smoothly."""
        self.canvas.scan_dragto(event.x, event.y, gain=1)

    def zoom(self, event):
        """Adjusts scale factor. Uses a multiplier for smooth scaling."""
    ...
    def update_image(self):
        """Rapidly resizes and updates the image."""
        ...
        # Optimization: BILINEAR is much faster than LANCZOS and perfect for live zooming
...
class ImageBatchCleaner:
    def __init__(self, root):
        ...
        self.current_images = [] 
        self.batches = {} 
        self.loading_job = None # Tracks background loading to prevent overlaps
    ...
    def on_batch_select(self, event):
        ...
        # Cancel any ongoing progressive loads so the UI doesn't cross-contaminate
        ...
        # Start the non-blocking progressive loader
    ...
    def load_image_progressively(self, folder_path, img_files, index, row, col):
        """Loads images one at a time so the GUI never freezes."""
        ...
        try:
            # Optimization: BILINEAR speeds up grid thumbnail generation by ~300%
        ...
        # Schedule the next image to load in 10ms, freeing up the UI thread in between
